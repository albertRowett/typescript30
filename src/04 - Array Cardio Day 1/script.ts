...
const inventorsBornIn1500s: Inventor[] = inventors.filter(
    (inventor: Inventor) => inventor.year > 1499 && inventor.year < 1600
);
...
const inventorNames: string[] = inventors.map((inventor: Inventor) => `${inventor.first} ${inventor.last}`);
console.log("Inventors' names:");
console.log(inventorNames);

// Array.prototype.sort()
// 3. Sort the inventors by birthdate, oldest to youngest
const inventorsSortedByBirthdate: Inventor[] = inventors.sort((a: Inventor, b: Inventor) => a.year - b.year);
console.log('Inventors sorted by birthdate (ascending):');
console.table(inventorsSortedByBirthdate);

// Array.prototype.reduce()
// 4. How many years did all the inventors live all together?
const totalYears: number = inventors.reduce((accumulator: number, inventor: Inventor) => {
...
const inventorsSortedByYearsLived: Inventor[] = inventors.sort(
    (a: Inventor, b: Inventor) => a.passed - a.year - (b.passed - b.year)
);
...
const counts: CountObject = data.reduce((accumulator: CountObject, current: string) => {
